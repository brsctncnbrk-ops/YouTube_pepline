...
export const FactForgeVideo:React.FC=()=>{const plan=editPlan as unknown as EditPlan;const captions=captionsData as unknown as CaptionsFile;const audioSrc=plan.audio_mix_asset||assetMap.audio_asset;return <AbsoluteFill style={{backgroundColor:"#05070C"}}><Audio src={staticFile(audioSrc)}/>{plan.shots.map((shot)=>{const durationInFrames=Math.max(1,shot.end_frame-shot.start_frame);const transitionIn=shot.transition_in||"cut";const transitionOut=shot.transition_out||"cut";return <Sequence key={shot.shot_id} from={shot.start_frame} durationInFrames={durationInFrames}>{shot.asset_type==="graphic"?<Scene assetType="graphic" graphic={shot.graphic} editorialOverlay={shot.editorial_overlay||null} durationInFrames={durationInFrames} textOverlay={shot.text_overlay||null} transitionIn={transitionIn} transitionOut={transitionOut}/>:shot.asset_type==="evidence"?<Scene assetType="evidence" evidence={shot.evidence} durationInFrames={durationInFrames} textOverlay={null} transitionIn={transitionIn} transitionOut={transitionOut}/>:<Scene assetType="footage" videoSrc={staticFile(shot.video_asset)} trimInSec={shot.trim_in_sec} trimOutSec={shot.trim_out_sec} motionVariant={shot.motion_variant||"hold"} editorialOverlay={shot.editorial_overlay||null} durationInFrames={durationInFrames} textOverlay={shot.text_overlay||null} transitionIn={transitionIn} transitionOut={transitionOut}/>}</Sequence>})}<CaptionLayer captions={captions.captions}/></AbsoluteFill>};
