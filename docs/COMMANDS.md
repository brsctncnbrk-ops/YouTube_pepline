...
node scripts/validate.mjs captions --project-id 001-my-video            # captions.json: schema-valid, chronological, readable-but-not-lingering
...
## Caption generation (used by the motion skill, after composition.json exists)

```bash
# Derives remotion/captions.json from scripts/script.md's section timestamps
# and composition.json's fps/duration_frames - mechanical, no ASR/forced
# alignment available, so timing is word-count-proportional (same method
# script.md's own section timestamps already use).
node scripts/generate_captions.mjs generate --project-id 001-my-video [--min-sec 0.9] [--max-sec 3.2] [--gap-frames 3]
```

## Remotion build helpers (used by the motion + editor skills)

```bash
# Derive scene_config.json + asset_map.json (+ merge captions.json) from the authored composition.json
