...
`assets/audio/final_voice.mp3`, and every `assets/images/scene_NNN.png`.
...
1. Assemble the render-ready project:
   ...
   refreshes `assets/asset_manifest.json` to reflect the real assets. Large
   binaries are referenced in place (the app's public dir points at the
   project root), never duplicated.

2. Check the helper's `missing_assets` in its JSON output. If it's non-empty,
   an image or the audio is missing even though the gates passed — stop and
   run `node scripts/manifest_cli.mjs error --project-id <project_id> --code MISSING_IMAGE --stage editor --message "<what's missing>" --action "Regenerate/replace the missing asset(s), then re-run the editor."` (use `MISSING_AUDIO` if it's the audio). Do not advance.
