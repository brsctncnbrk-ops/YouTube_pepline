...
export async function bindFullRenderEvidenceBundle(projectId = PROJECT_ID) {
  ...
  for (const assetId of usedIds) {
    ...
    if (!declared) throw new Error(`${assetId} is used but missing from the evidence manifest`);
    if (!produced) throw new Error(`${assetId} is used but missing from the runtime evidence manifest`);
    ...
    bundledAssets.push({ evidence_asset_id: assetId, local_asset: localAsset, source_url: declared.source_url, sha256: produced.sha256, bytes: produced.bytes, width: produced.width, height: produced.height, source_ids: declared.source_ids || [] });
  }
  manifest.schema_version = "2.7-deterministic-full-render-evidence-bundle";
  manifest.policy = {
    ...(manifest.policy || {}),
  ...
  const report = {
    schema_version: "1.0-deterministic-full-render-evidence-bundle",
    project_id: projectId,
    generated_at: new Date().toISOString(),
    used_evidence_asset_count: usedIds.length,
    bundled_asset_count: bundledAssets.length,
    local_assets: [...new Set(localAssets)],
    runtime_manifest: "assets/evidence/primary_evidence.runtime.json",
    assets: bundledAssets,
    pass: bundledAssets.length === usedIds.length,
  };
  await Promise.all([
