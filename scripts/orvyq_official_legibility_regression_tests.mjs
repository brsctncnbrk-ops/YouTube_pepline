...
const rebalance = 'node scripts/orvyq_rebalance_full_plan_v3.mjs "$PROJECT_ID"';
...
assert.ok(rebalanceIndex > preflightIndex, "Three-phase rebalance must run after read-only preflight");
assert.ok(finalIndex > rebalanceIndex, "Final official normalization must run after evidence-chain rebalance");
...
assert.match(normalizer, /plan_mutated: mode === "final"/, "Report must disclose whether the plan was mutated");
assert.match(normalizer, /prefixHashBefore !== prefixHashAfter/, "Normalizer must preserve the approved proof prefix");
assert.match(rebalancer, /safe_breaker_recovery/, "Rebalancer must recover the official floor after breaking evidence chains");
assert.match(rebalancer, /maximum_uninterrupted_evidence_seconds/, "Rebalancer must retain the evidence-run ceiling");
assert.match(rebalancer, /source_mix_pending/, "Rebalancer must explicitly hand off the source-mix phase");

console.log(JSON.stringify({
  ok: true,
  contract: "orvyq-three-phase-structural-rebalance-v3",
