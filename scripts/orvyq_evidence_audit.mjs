...
export async function runEvidenceAudit(projectId = PROJECT_ID) {
  ...
  for (const shot of plan.shots) {
    ...
    for (const sourceId of overlaySources) {
      if (!sourceById.has(sourceId)) failures.push(`${shot.shot_id} references unknown source ${sourceId}`);
    }
    if (["evidence", "archive"].includes(shot.visual_role) && overlaySources.length === 0 && !shot.evidence_asset) {
      failures.push(`${shot.shot_id} is marked ${shot.visual_role} but declares neither source_ids nor an evidence_asset`);
    }
  }
  ...
  for (const claimId of activeClaimIds) {
    ...
    const evidenceShots = shots.filter((shot) =>
      ["evidence", "archive"].includes(shot.visual_role)
      || (shot.editorial_overlay?.source_ids || []).length > 0
      || shot.asset_type === "graphic"
    );
    const usedSourceIds = unique(shots.flatMap((shot) => shot.editorial_overlay?.source_ids || []));
    const declaredSources = claim.source_ids || [];
    const missingDeclaredSources = declaredSources.filter((sourceId) => !sourceById.has(sourceId));
    ...
    if (claim.status === "attributed_commentary" && shots.every((shot) => !shot.editorial_overlay)) {
      warnings.push(`${claimId} is attributed commentary but has no explicit editorial/source context overlay`);
    }
    ...
    claimReports.push({
      ...
      evidence_shot_count: evidenceShots.length,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runEvidenceAudit().then((report) => console.log(JSON.stringify({ ok: true, ...report }))).catch((error) => {
