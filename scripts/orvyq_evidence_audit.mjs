...
export async function runEvidenceAudit(projectId = PROJECT_ID) {
  ...
  const sourceById = new Map(
    map.source_catalog.map((source) => [source.source_id, source]),
  );
  ...
  for (const shot of plan.shots) {
    ...
    if (
      ["evidence", "archive"].includes(shot.visual_role) &&
      !isClaimSpecificEvidence(shot)
    )
      failures.push(
        `${shot.shot_id} is marked ${shot.visual_role} but is not a source-backed evidence asset`,
      );
    if (
      plan.preview &&
  ...
  for (const claimId of activeClaimIds) {
    ...
    const missingDeclaredSources = (claim.source_ids || []).filter(
      (sourceId) => !sourceById.has(sourceId),
    );
    if (missingDeclaredSources.length)
      failures.push(
        `${claimId} has unknown declared sources: ${missingDeclaredSources.join(", ")}`,
      );
    ...
    const evidencePass =
      ...
      (!critical ||
        ...
        claim.status === "attributed_commentary");
    if (evidencePass) evidenceWeight += weight;
    else
      failures.push(
        `${claimId} has no physical, source-backed visual evidence`,
      );
    if (
      claim.status === "attributed_commentary" &&
      !shots.some((shot) => sourceIdsFor(shot).length)
    )
      warnings.push(
        `${claimId} is attributed commentary but has no explicit source context`,
      );
    claimReports.push({
      claim_id: claimId,
      status: claim.status,
      ...
      physical_evidence_shot_count: evidenceShots.length,
      source_ids_used: usedSourceIds,
  ...
  const minimum = plan.preview
    ? 0.9
    : Number(
        map.full_render_gate
          ?.minimum_weighted_verified_or_attributed_coverage || 0.9,
      );
  if (supportedCoverage < minimum)
    failures.push(
      `weighted source coverage ${(supportedCoverage * 100).toFixed(1)}% is below ${(minimum * 100).toFixed(1)}%`,
    );
  if (visualEvidenceCoverage < minimum)
    failures.push(
      `weighted physical visual-evidence coverage ${(visualEvidenceCoverage * 100).toFixed(1)}% is below ${(minimum * 100).toFixed(1)}%`,
    );
  if (!plan.preview) {
    const unresolved = map.claims.filter((claim) =>
      ["rewrite_required", "source_required"].includes(claim.status),
    );
    if (unresolved.length)
      failures.push(
        `full render blocked by unresolved claims: ${unresolved.map((claim) => claim.claim_id).join(", ")}`,
      );
  }
  const report = {
    schema_version: "2.0-physical-evidence",
  ...
  if (!report.pass)
    throw new Error(`ORVYQ evidence audit failed: ${failures.join("; ")}`);
  return report;
}
