...
export async function runMusicCueAudit(projectId = PROJECT_ID) {
  ...
  if (plan.preview) {
    ...
    if (!contiguous(activeCues, plan.duration_frames / plan.fps))
      failures.push("proof music sections do not continuously cover the proof duration");
  } else {
    activeCues = cueSheet.full_cues || [];
    if (!contiguous(activeCues, cueSheet.duration_seconds))
    ...
    if (incomplete.length)
      failures.push(
        `full music cues are not ready: ${incomplete.map((cue) => cue.cue_id).join(", ")}`,
      );
  ...
  const continuousCoverage = contiguous(
    activeCues,
    plan.preview ? plan.duration_frames / plan.fps : cueSheet.duration_seconds,
  );
  const report = {
    schema_version: "2.0-policy-aware-audio",
