...
async function command(binary, args) {
  try {
    return await exec(binary, args, { maxBuffer: 48 * 1024 * 1024 });
  } catch (error) {
...
async function generateOriginalSectionedScore(musicDir) {
  ...
  const sections = [
    {
      frequencies: [55, 73.42, 82.41, 110],
      volumes: [0.19, 0.12, 0.075, 0.04],
      tremolo: [0.12, 0.16, 0.21, 0.27],
      depth: 0.16,
      lowpass: 1250,
      echo: "640|1280:0.12|0.06",
      gain: 0.86,
    },
    {
      frequencies: [58.27, 87.31, 116.54, 174.61],
      volumes: [0.16, 0.105, 0.065, 0.035],
      tremolo: [0.18, 0.23, 0.31, 0.38],
      depth: 0.2,
      lowpass: 1850,
      echo: "430|860:0.13|0.07",
      gain: 0.82,
    },
    {
      frequencies: [49, 73.42, 98, 146.83],
      volumes: [0.21, 0.13, 0.075, 0.035],
      tremolo: [1.5, 0.75, 0.38, 0.24],
      depth: 0.32,
      lowpass: 1500,
      echo: "360|720:0.11|0.055",
      gain: 0.93,
    },
    {
      frequencies: [65.41, 82.41, 110, 164.81],
      volumes: [0.13, 0.09, 0.055, 0.032],
      tremolo: [0.09, 0.13, 0.17, 0.21],
      depth: 0.12,
      lowpass: 2100,
      echo: "720|1440:0.14|0.065",
      gain: 0.72,
    },
  ];
  ...
  sections.forEach((section, sectionIndex) => {
    ...
    section.frequencies.forEach((frequency, toneIndex) => {
      ...
      filters.push(`[${inputIndex}:a]volume=${section.volumes[toneIndex]},tremolo=f=${section.tremolo[toneIndex]}:d=${section.depth}[${label}]`);
      labels.push(`[${label}]`);
      inputIndex += 1;
    });
    filters.push(
      `${labels.join("")}amix=inputs=4:normalize=0,highpass=f=32,lowpass=f=${section.lowpass},` +
      `aecho=0.75:0.5:${section.echo},volume=${section.gain},aformat=channel_layouts=stereo[section${sectionIndex}]`,
    );
  });
  ...
  await command("ffmpeg", [
    ...
    "-map", "[bed]", "-t", String(PREVIEW_SCORE_SECONDS),
...
async function prepareNarrator({ dir, audioDir, sourceVoice, sourceDuration }) {
  ...
  return {
    ...
    repair: { operation: "rotate", rotate_at_seconds: rotateAt, config: "voice/audio_repair.json", reason: repair.reason || null },
  };
}
...
export async function buildOrvyqAudioMix(projectId = PROJECT_ID) {
  ...
  if (!hasApprovedMusic && outputDuration > PREVIEW_SCORE_SECONDS + 0.1) {
    throw new Error("Full ORVYQ render requires an approved full-duration music bed or an explicit full cue sheet; repeating the two-minute proof score is forbidden");
  }
  ...
  await writeJsonAtomic(path.join(audioDir, "final_mix.metadata.json"), {
    ...
    music_origin: hasApprovedMusic ? "user-approved licensed asset" : "original sectioned ORVYQ score generated from harmonic oscillators only",
    music_sections: hasApprovedMusic ? [{ id: "approved_full_bed", start: 0, end: outputDuration, purpose: "User-approved full mix" }] : MUSIC_SECTIONS,
    ...
    licensing: hasApprovedMusic
      ...
      : "Narration plus an original four-movement ORVYQ score generated locally without noise sources or third-party audio.",
  });
