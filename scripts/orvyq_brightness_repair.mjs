...
import {
  detectTransientBrightnessDrops,
  parseLumaSamples,
} from "./orvyq_media_qa.mjs";
...
async function scan(video) {
  const { stdout, stderr } = await command("ffmpeg", [
    "-hide_banner",
    "-nostats",
    "-i",
    video,
    "-an",
    "-vf",
    `fps=${FPS},signalstats,metadata=print:key=lavfi.signalstats.YAVG`,
    "-f",
    "null",
    "-",
  ]);
  ...
  return {
    ...
    duration,
    drops: detectTransientBrightnessDrops(samples, duration, {
      sampleInterval: 1 / FPS,
    }),
  };
}
...
function frameRanges(drops) {
  ...
  const padded = drops.map((drop) => {
    ...
    const last = Math.max(
      first,
      Math.round(drop.end * FPS) - 1 + paddingFrames,
    );
    return { first, last };
  });
  const merged = [];
...
export async function repairBrightnessDrops({ projectId, video, reportPath }) {
  ...
  if (ranges.length) {
    ...
    const rejectedFrames = ranges
      .map(({ first, last }) => `between(n\\,${first}\\,${last})`)
      .join("+");
  ...
  const report = {
    schema_version: "1.0-isolated-frame-repair",
    ...
    before: {
      sample_count: before.samples,
      transient_brightness_drops: before.drops,
    },
    after: {
      sample_count: after.samples,
      transient_brightness_drops: after.drops,
    },
  ...
  if (!report.pass) {
    throw new Error(
      `Brightness repair left ${after.drops.length} transient drops`,
    );
  }
  return report;
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!projectId || !video) {
    console.error(
      "Usage: node scripts/orvyq_brightness_repair.mjs --project-id <id> --video <path> [--report <path>]",
    );
    process.exitCode = 1;
  } else {
    const reportPath =
      args.report ||
      path.join(projectDir(projectId), "qa", "orvyq_brightness_repair.json");
