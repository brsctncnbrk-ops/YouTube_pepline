...
export async function buildOrvyqEditPlan(projectId = PROJECT_ID) {
  ...
  if (full.status !== "ready") {
    throw new Error(`Full ORVYQ edit is blocked: full_production.status=${full.status}`);
  }
  if ((full.blocking_claim_ids || []).length || unresolved.length) {
    throw new Error(`Full ORVYQ edit is blocked by unresolved claims: ${[...(full.blocking_claim_ids || []), ...unresolved.map((claim) => claim.claim_id)].join(", ")}`);
  }
  ...
  for (let index = 0; index < full.shots.length; index += 1) {
    ...
    if (!claimIds.has(spec.claim_id)) throw new Error(`full_production.shots[${index}] has unknown claim_id ${spec.claim_id}`);
    ...
    if (sourceUsage.get(spec.asset) > blueprint.global_rules.max_uses_per_source) {
      throw new Error(`${spec.asset} exceeds the ${blueprint.global_rules.max_uses_per_source}-use limit`);
    }
  ...
  if (Math.abs(cursor * composition.fps - composition.duration_frames) > 1) {
    throw new Error(`Explicit full edit totals ${cursor.toFixed(3)}s, but composition is ${(composition.duration_frames / composition.fps).toFixed(3)}s`);
  }

  const plan = {
    schema_version: "5.0-evidence-led-full",
...
if (import.meta.url === `file://${process.argv[1]}`) {
  buildOrvyqEditPlan().then((plan) => console.log(JSON.stringify({
    ok: true,
