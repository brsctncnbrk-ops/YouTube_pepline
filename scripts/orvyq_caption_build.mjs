...
function normalizeToken(token) {
  return cleanDisplayToken(token)
    .toLowerCase()
    .replace(/[’]/g, "'")
    .replace(/[^a-z0-9']/g, "")
    .replace(/^'+|'+$/g, "");
...
function mergeRecognizedWords(words) {
  ...
  for (const word of words || []) {
    ...
    merged.push({
      ...
      probability: Number(word.probability ?? 1),
    });
  }
...
function scriptWords(text) {
  return text
    ...
    .trim()
    .split(" ")
    .map((token) => ({ text: cleanDisplayToken(token), normalized: normalizeToken(token) }))
...
export async function buildOrvyqCaptions(projectId = PROJECT_ID) {
  ...
  const [composition, speechQa, approvedScript, audioMetadata] = await Promise.all([
  ...
  const timedScript = interpolateUnmatched(alignment.target, alignment.mapping, speechWords)
    .filter((word) => Number(word.start) < maxSeconds && Number(word.start) < Number(audioMetadata.narration_duration_seconds ?? maxSeconds));
